...
# Define default path to pg_basebackup binary.  We assume that binaries have
# been installed using PGDG RedHat/CentOS packaging default location.  If that
# is not the case, it is necessary to use a distribution agnostic
# initialization script.
# FIXME pg_basebackup would normally be in the PATH
pgbasebackup="/usr/pgsql-${pgversion}/bin/pg_basebackup"

if [ ! -x "$pgbasebackup" ]; then
    die "\"${pgbasebackup}\" is not installed."
...
"$pgbasebackup" -w -U "$repli_user" -c "fast" -h "$primary_host" "$wal_get_method" -D "$pgdata_dir"
